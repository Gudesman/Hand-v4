...
#define SIZEOF_VARRAY (NOF*NOJ*8)//数组4*4*8
...
public:
	...
	/**
	 * Set joint position of each joint.设置每一个关节 
	 * The hand has sixteen joints totally. 
	 * The dimension of 'q' should be sixteen.q是16维
	 ...
	 *更新回调控制算法，每个控制时间步手动调整
	 * Desired joint torques are computed once in every control loop.期望关键扭矩
	 * @param time Current time in second.
	 */
	void UpdateControl(double time);//更新回调控制算法
	 ...
	 */
	void GetJointTorque(double* tau);//期望关节扭矩
	 ...
	 */
	void GetFKResult(double x[4], double y[4], double z[4]);//得到每个手指的正向运动学解

	/**
	 * Set desired joint position.
	 * @param q Array of desired joint positions in radian.
	 */
	void SetJointDesiredPosition(double* q);//设置期望关节位置

	/**
	 * Set control gains explicitly./明确地设置控制增益
	 * @param kp proportional gains//比例增益
	 * @param kd derivative gains//导数增益
	 */
	void SetGainsEx(double* kp, double* kd);//设置控制增益

	/**
	 * Set desired grasping forces.
	 * @param f desired grasping force for each finger
	 */
	void SetGraspingForce(double f[4]);//设置每根手指的期望抓力
	 ...
	 */
	void SetEnvelopTorqueScalar(double set_scalar=1.0);//给预设包裹抓力的力矩设置标量
	 ...
	 */
	void GetGraspingForce(double fx[4], double fy[4], double fz[4]);//获取每根手指的期望抓力

	/**
	 * An attempt on object moving
	 */
	void SetObjectDisp(double x_d_o[3]);//尝试物体移动
	 ...
	 * Set orientation
	 */
	void SetOrientation(double roll, double pitch, double yaw);//设置方向摇晃、仰俯、偏航
